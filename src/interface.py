...
def show_video_data():
    ...
    if downloadFile.link == "":
        ...
        if video:
            ...
            image = image.resize(
                (350, int(image.height * (350 / image.width))), Image.BILINEAR
            )
...
def display_app(app):
    ...
    button_OK = customtkinter.CTkButton(
        app, text="OK", width=10, command=show_video_data
    )
    ...
    video_name = customtkinter.CTkTextbox(
        app,
